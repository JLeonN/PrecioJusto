...
- [ ] Definir funciones claras (en `BuscadorProductosService.js` **o** módulo dedicado p.ej. `BusquedaProductosHibridaService.js`) — elegir un solo lugar documentado:
  - [ ] `buscarPorCodigoConPolitica(codigo, opciones)` → devuelve estructura unificada: `{ origen: 'local' \| 'api' \| 'mixto', productoLocal, resultadoApi, itemsParaDialogo }` (ajustar forma al `DialogoResultadosBusqueda` existente).
  - [ ] Comportamiento según decisiones de producto (MVP) arriba.

### 1.2 — Diseño de API (nombre)

- [ ] `buscarPorNombreConPolitica(texto, opciones)` con:
  - [ ] Paso local vía `ProductosService.buscarPorNombre` (o alineación con criterio de similitud si hace falta).
  - [ ] Paso OFF solo cuando corresponda (cero locales, o usuario pidió ampliar).
  - [ ] Límite razonable de resultados y normalización de campos para el diálogo de resultados.

### 1.3 — Documentación en código

- [ ] Comentario breve en el servicio: política MVP + enlace a este plan.

### Testing (Fase 1)

- [ ] Pruebas manuales desde consola o componente temporal: código existente solo local, código solo API, código inexistente en ambos.
- [ ] Nombre con coincidencias locales, sin coincidencias, y “ampliar a OFF”.
...
- [ ] Sustituir llamada directa solo-API por flujo híbrido.
- [ ] Si resultado es solo local: abrir `DialogoResultadosBusqueda` con ítem(es) locales o autocompletar según UX acordada.
- [ ] Botón o acción explícita para disparar cadena API si ya hay local.
- [ ] `:loading` / estado visual mientras corre la parte de red.

### 2.2 — Buscar por nombre

- [ ] Mismo patrón: resultados locales primero; OFF bajo reglas MVP.
- [ ] Debounce opcional si en el futuro la búsqueda se dispara en vivo; si sigue siendo “al pulsar buscar”, documentar que no hace falta.

### 2.3 — `DialogoResultadosBusqueda`

- [ ] Si hace falta: distinguir visualmente origen local vs API (caption discreto o sección) para evitar confusión.

### Testing (Fase 2)

- [ ] Flujo completo agregar producto: código nuevo, código ya guardado, nombre con match local, nombre sin match (cae a OFF).
...
- [ ] Aplicar la misma regla “no llamar API si local suficiente” que en el diálogo (misma función de política si es posible).
- [ ] Indicador no intrusivo mientras espera API (toast, overlay en `TarjetaEscaneo`, o estado en la tarjetita de aviso) — elegir una opción y unificar criterio con Fase 4.

### 3.2 — Mesa de trabajo / bandeja

- [ ] Verificar si algún camino dispara búsqueda API al editar items; alinear con política o documentar excepción.

### Testing (Fase 3)

- [ ] Ráfaga con código ya en base local: no debe disparar cadena API completa si MVP lo prohíbe.
- [ ] Código desconocido localmente: sí API + loading perceptible.
...
- [ ] Listar quién lee `productosStore.cargando` en plantillas.
- [ ] Introducir `buscandoEnApi` / `buscandoProductosExterno` a nivel store **o** estado local solo en páginas/diálogos — preferir la opción con menos acoplamiento.

### 4.2 — Patrón UX recomendado

- [ ] Fase A (instantánea): sin spinner global; opcional texto “Buscando en tus productos…” solo si la local pudiera ser lenta (volumen grande).
- [ ] Fase B (red): spinner en botón que disparó la acción o `q-inner-loading` en la card del diálogo.
- [ ] Errores de red: mantener `notify` existente; no dejar el botón colgado en loading.

### Testing (Fase 4)

- [ ] Con lista grande de productos, abrir diálogo agregar y buscar: la lista principal no debe parpadear en “Cargando productos…” salvo que sea carga inicial real.

═══════════════════════════════════════════════════════════════

## FASE 5: DOCUMENTACIÓN

- [ ] Actualizar [Resumen1General.md](Resumenes/Resumen1General.md) (flujo de búsqueda / capa de servicios) si el contrato público cambia.
- [ ] Actualizar [Resumen6OpenFoodFacts.md](Resumenes/Resumen6OpenFoodFacts.md) si la búsqueda por texto queda condicionada a botón explícito o orden local primero.
- [ ] En [TareasPendientes.md](PlanesFuturos/TareasPendientes.md): tachar o eliminar los dos ítems cuando el plan esté implementado y testeado, o dejar referencia “ver PlanBusquedaLocalPrimeroYEstadosCarga” si queda trabajo menor.
...
- [ ] Código presente solo en local: no se llama cadena API sin acción explícita del usuario (MVP).
- [ ] Código ausente en local: se llama orquestador; resultado coherente con fuentes y `fuenteDato`.
- [ ] Código ISBN 978/979: comportamiento alineado (libros) tras refactor.

### T.B — Política nombre

- [ ] Texto con matches locales: se listan locales; OFF no se dispara solo en ese paso.
- [ ] Texto sin matches locales: OFF se dispara con loading visible.
- [ ] Usuario pide “buscar en OFF” con matches locales: resultados OFF se muestran sin perder claridad de origen.

### T.C — Carga y percepción

- [ ] Ningún flujo deja la UI sin feedback >300 ms en operación de red típica.
- [ ] Cancelar / cerrar diálogo durante búsqueda: sin errores en consola ni estados colgados.

### T.D — Red y errores

- [ ] Sin conexión: mensaje claro; no crash; loading se apaga en `finally`.
- [ ] Timeout / 404 API: mismo criterio.

### T.E — Condiciones de carrera

- [ ] Doble clic rápido en “Buscar”: una sola respuesta efectiva o última gana sin duplicar diálogos incoherentes.

### T.F — Plataforma

- [ ] Android (APK): UPCitemdb en cadena cuando corresponde.
- [ ] Navegador dev: comportamiento documentado si UPC no aplica (CORS).

### T.G — Regresión

- [ ] Guardar producto nuevo y existente desde diálogo sin regresiones.
- [ ] “Restaurar desde API” en detalle sigue funcionando.
